...
exports.GetByID = function(user_id, callback) {
    console.log(user_id);
    var query = 'SELECT * FROM rating JOIN user ON user.user_id = rating.user_id \n' +
        'JOIN movie ON movie.movie_id = rating.movie_id WHERE rating.user_id =' + user_id;
    console.log(query);
...
exports.Insert = function(user_info, callback) {
     ...
     */
    var dynamic_query = 'INSERT INTO user (first_name, last_name, email, pass_word) VALUES (' +
        '\'' + user_info.firstname + '\', ' +
        '\'' + user_info.lastname + '\', ' +
        '\'' + user_info.email + '\', ' +
        '\'' + user_info.password + '\'' +
